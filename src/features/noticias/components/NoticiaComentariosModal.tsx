// src/features/noticias/components/NoticiaComentariosModal.tsx
import { useEffect, useMemo, useState } from "react";
import {
  deleteNoticiaComentario,
  getNoticiaComentarios,
  toggleAprobacionNoticiaComentario,
...
export function NoticiaComentariosModal({ open, onOpenChange, noticiaId, onChanged }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    load();
  }, [open, noticiaId, soloAprobados]);

  const pendientes = useMemo(() => items.filter((i) => !i.aprobado).length, [items]);

  const getAutor = (c: NoticiaComentario) => {
    if (c?.usuario) {
      const n = c.usuario.nombres ?? c.usuario.nombre ?? "";
      const a = c.usuario.apellidos ?? "";
      const full = `${n} ${a}`.trim();
      return full || "Anónimo";
    }
    return c.nombreAutor ?? "Anónimo";
  };

  const getFecha = (c: NoticiaComentario) => {
